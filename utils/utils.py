...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    if (len(label) == 2):
        ...
        c2 = c1[0] + t_size[0] - 20, c1[1] - t_size[1] - 3
        cv2.rectangle(img, c1, c2, color, -1, cv2.LINE_AA)  # filled
        # fontpath = "/home/ly/code/Traffic-Light-Detection-Using-YOLOv3-master/simsun.ttc"  # 宋体字体文件
        # font_1 = ImageFont.truetype(fontpath, 1)  # 加载字体, 字体大小
        # img_pil = Image.fromarray(img)
        # draw = ImageDraw.Draw(img_pil)
        # draw.text((c1[0], c1[1] - 2), label, font=font_1, fill=(225, 255, 255))  # xy坐标, 内容, 字体, 颜色
        # img = cv2.cvtColor(numpy.asarray(img_pil),cv2.COLOR_RGB2BGR)
        # img = cv2AddChineseText(img, label, (c1[0], c1[1]-15), (225, 255, 255), 15)
    else:
        tf = max(tl - 1, 1)  # font thickness
        t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
        c2 = c1[0] + t_size[0] - 85, c1[1] - t_size[1] - 3
        cv2.rectangle(img, c1, c2, color, -1, cv2.LINE_AA)  # filled
    img = cv2AddChineseText(img, label, (c1[0], c1[1] - 15), (225, 255, 255), 15)
...
def cv2AddChineseText(img, text, position, textColor=(225, 255, 255), textSize=30):
    if (isinstance(img, np.ndarray)):  # 判断是否OpenCV图片类型
        img = Image.fromarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
    # 创建一个可以在给定图像上绘图的对象
    draw = ImageDraw.Draw(img)
    # 字体的格式
    fontpath = "/home/ly/code/Traffic-Light-Detection-Using-YOLOv3-master/simsun.ttc"
